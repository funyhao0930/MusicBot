...
class SharedWebUiParityTests(unittest.TestCase):
    def test_local_queue_rows_expose_the_public_reorder_controls(self) -> None:
        ...
        self.assertIn("if (state.draggingIndex === null || state.mutationBusy) return;", local_app)
        self.assertIn("if (state.mutationBusy || !count) return;", local_app)
        self.assertIn('api("/api/queue/play"', local_app)
        self.assertIn('class="queue-copy queue-play-target"', local_app)
        self.assertIn('"is-drop-before", "is-drop-after"', local_app)
    ...
    def test_local_shared_controls_keep_the_public_responsive_visual_contract(self) -> None:
        ...
        self.assertIn(".queue-item.is-drop-before::before", local_css)
